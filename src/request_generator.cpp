...
uint64_t Request::getArrival() const noexcept { return tillArrival; }
...
bool Request::isReserved() const noexcept { return tillArrival > 0; }
...
Requests RequestGenerator::generate() {
    ...
    for (uint64_t i = 0; i < count; ++i) {
        requests.emplace_back(dropoffDist(rng), durationDist(rng), arrivalDist(rng));
    }
